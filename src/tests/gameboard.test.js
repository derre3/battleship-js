...
test('place ship size 4 at grid edge [7,3]', () => {
  const board = Gameboard(8);
  const ship = Ship(4);
  expect(board.placeShipAt(ship, [7, 3], 1)).toBe('invalid');
...
test('place vertical ship size 5 at grid edge [3,7]', () => {
  const board = Gameboard(8);
  const ship = Ship(5);
  expect(board.placeShipAt(ship, [3, 7], 0)).toBe('invalid');
...
test('do not overwrite ship position while placing it on the board', () => {
  ...
  const ship1 = Ship(1);
  const ship2 = Ship(3);
  board.placeShipAt(ship1, [2, 0], 1);
  board.placeShipAt(ship2, [0, 0], 1);
